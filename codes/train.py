                    ...
                    with torch.no_grad():
                        for idx, (img, lab) in enumerate(val_loader, start=1):
                            ...
                            if cfg.save_pos_acc < (val_acc['mask_pos_acc'] / idx) and cfg.save_acc < (
                                ...
                                #trainer.saver.save(model, tag='%s_epoch(%d)_acc(%0.5f/%0.5f)' % (cfg.name, epoch, cfg.save_pos_acc, cfg.save_acc))
                                #trainer.vis.log('Save Model %s_epoch(%d)_acc(%0.5f/%0.5f)' % (cfg.name, epoch, cfg.save_pos_acc, cfg.save_acc), 'train info')

                                trainer.saver.save(model, tag='%s_epoch_accData(%d)' % (cfg.name, epoch))
                                trainer.vis.log('Save Model %s_epoch_accData(%d)' % (cfg.name, epoch), 'train info')
                                #trainer.saver.save(model, tag='%s_HoitData(%d)' % (cfg.name, epoch))
                                #trainer.vis.log('Save Model %s_HoitData(%d)_acc(%0.5f/%0.5f)' % (
